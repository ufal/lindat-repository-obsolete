...
/**
 * based on clas by Lyncode Development Team <dspace@lyncode.com>
...
public class XOAI {
    ...
    private int index(Date last) throws DSpaceSolrIndexerException {
        ...
        String sqlQuery = "SELECT item_id FROM item WHERE (in_archive=TRUE OR withdrawn=TRUE) AND last_modified > ?";
    ...
    private int indexAll() throws DSpaceSolrIndexerException {
        ...
        try {
            ...
            String sqlQuery = "SELECT item_id FROM item WHERE in_archive=TRUE or withdrawn=TRUE";
            if(DatabaseManager.isOracle()){
