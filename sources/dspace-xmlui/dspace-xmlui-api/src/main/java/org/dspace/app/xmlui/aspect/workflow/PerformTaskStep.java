 ...
 * metadata before accepting or rejecting. The user is also given the option of
 * taking the task or returning it to the pool.
 * 
...
public class PerformTaskStep extends AbstractStep
{
    ...
    protected static final Message T_curate_help = message("xmlui.Submission.workflow.PerformTaskStep.curate_help");

    protected static final Message T_approve_curate = message("xmlui.Submission.workflow.PerformTaskStep.curate_submit");
    ...
    // UFAL
    protected static final Message T_record_format_help = message("xmlui.Submission.workflow.PerformTaskStep.record_format_help");

    // UFAL
    protected static final Message T_label_move = message("xmlui.administrative.item.EditItemStatusForm.label_move");

    protected static final Message T_submit_move = message("xmlui.administrative.item.EditItemStatusForm.submit_move");

    protected static final Message T_head_parent_collections = message("xmlui.ArtifactBrowser.ItemViewer.head_parent_collections");

    /**
     * Copy the workflow manager's state values so that we can reference them
     * easier.
     */
    ...
    {
        // <UFAL>
        // Get any metadata that may be removed by unselecting one of these
        // options.
        ...
        DCValue[] dcv = item.getMetadata("local.submission.note");
        ...
        if (state == WFSTATE_STEP1POOL || state == WFSTATE_STEP2POOL
        ...
        if (state == WFSTATE_STEP1 || state == WFSTATE_STEP2)
        {
            ...
            row.addCell().addButton("submit_approve")
                    .setValue(T_approve_submit);
            // Check task
            row = table.addRow();
