...
/**
 * Display a form that allows the user to edit their profile. There are two
 * cases in which this can be used: 1) when an existing user is attempting to
 * edit their own profile, and 2) when a new user is registering for the first
 * time.
 *
 ...
 *
 * registering - A boolean value to indicate whether the user is registering for
 * the first time.
 *
 * retryInformation - A boolean value to indicate whether there was an error
 * with the user's profile.
 *
 * retryPassword - A boolean value to indicate whether there was an error with
 * the user's password.
 *
 * allowSetPassword - A boolean value to indicate whether the user is allowed to
...
public class EditProfile extends AbstractDSpaceTransformer {
	...
	private static final Message T_login = message("xmlui.EPerson.EditProfile.last_login");
    ...
    public void setup(SourceResolver resolver, Map objectModel, String src,
		...
		this.registering = parameters.getParameterAsBoolean("registering",
				false);
		this.allowSetPassword = parameters.getParameterAsBoolean(
    ...
    public void addBody(Body body) throws WingException, SQLException {
		...
		boolean has_password = eperson.getPasswordHash() != null
				&& !eperson.getPasswordHash().isEmpty();
	 ...
	 * property xmlui.supported.locales returns an array of Locales or null
	 *
