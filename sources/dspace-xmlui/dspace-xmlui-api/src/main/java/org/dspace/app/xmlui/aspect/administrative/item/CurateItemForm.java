...
public class CurateItemForm extends AbstractDSpaceTransformer {
	...
	/**
	 * common package method for initializing form gui elements Could be
	 * refactored.
	 * 
	 * @param pageMeta
	...
	public void addBody(Body body) throws WingException, SQLException,
		...
		String curateGroup = "";
		try {
			curateGroup = (parameters.getParameter("select_curate_group") != null) ? parameters
					.getParameter("select_curate_group")
					: FlowCurationUtils.UNGROUPED_TASKS;
		} catch (Exception pe) {
			// noop
		}
		if (!FlowCurationUtils.groups.isEmpty()) {
			curationTaskList.addLabel(T_taskgroup_label_name); // needs to check
																// for >=1 group
																// configured
			Select groupSelect = curationTaskList.addItem().addSelect(
					"select_curate_group");
			...
			if (curateGroup.equals("")) {
				curateGroup = (String) (FlowCurationUtils.groups.keySet()
						.iterator().next());
			}
		...
		taskSelect = FlowCurationUtils.getTaskSelectOptions(taskSelect,
				curateGroup);
	...
	// Add a method here to build it into the dspace.cfg ... ui.curation_tasks =
	// estimate = "Estate"
