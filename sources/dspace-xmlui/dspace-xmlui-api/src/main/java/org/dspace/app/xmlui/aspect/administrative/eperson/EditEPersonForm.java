...
public class EditEPersonForm extends AbstractDSpaceTransformer
{
    ...
    private static final Message T_welcome_message =
            message("xmlui.administrative.eperson.AddEPersonForm.welcome_message");
	...
	//
	//
	private void add_config_info(Request request, EPerson edited_eperson, List identity, java.util.List<String> errors) throws SQLException, WingException
	{
		...
		{
		    identity.addItem().addHidden("language").setValue((languageValue == null || languageValue.equals("")) ? I18nUtil.DEFAULTLOCALE.toString() : languageValue);
		}
		else
		{
    		if (isAdmin())
            {
            	...
            	language.setOptionSelected((languageValue == null || languageValue.equals("")) ? I18nUtil.DEFAULTLOCALE.toString() : languageValue);

            }else {
            	identity.addLabel(T_language);
            	//identity.addItem(languageValue);
    	        identity.addItem((languageValue == null || languageValue.equals("")) ? I18nUtil.DEFAULTLOCALE.toString() : languageValue);
            }
	    }
        ...
        if (isAdmin())
        {
	        ...
	        CheckBox canLogInField = identity.addItem(
	        		null, "admin-field").addCheckBox("can_log_in");
	        canLogInField.setLabel(T_can_log_in);
	        canLogInField.addOption(canLogInValue, "true");

	        CheckBox certificateField = identity.addItem(
	        		null, "admin-field").addCheckBox("certificate");
	...
	private void add_general_info(Request request, EPerson edited_eperson, List identity, java.util.List<String> errors)
			throws SQLException, WingException
	{
		...
		IFunctionalities functionalitiesManager = DSpaceApi.getFunctionalityManager();
		functionalitiesManager.openSession();

		// in case the user is still not registered, do it
		// ?? - legacy because of updates?
		UserRegistration userReg = functionalitiesManager.getRegisteredUser( edited_eperson.getID() );
		if(userReg==null && edited_eperson.getEmail()!=null && !edited_eperson.getEmail().equals("")) {
			userReg = functionalitiesManager.registerUser(edited_eperson.getID(), edited_eperson.getEmail(), "anonymous", true);
		}
		////
	    ...
	    add_key_pair(
	    		isAdmin(), true, "email_address", T_email_address, emailValue, identity,
	    		get_error(errors,
	    				new String[] {"eperson_email_key", "email_address"},
	    				new Message[] {T_error_email_unique, T_error_email})
	    			);
	    add_key_pair(
	    		isAdmin(), true, "first_name", T_first_name, firstValue, identity,
	    		get_error(errors, "first_name", T_error_fname ) );
	    add_key_pair(
	    		isAdmin(), true, "last_name", T_last_name, lastValue, identity,
	    		get_error(errors, "last_name", T_error_lname ) );
        add_key_pair(
                true, true, "org", T_org, orgValue, identity,
                get_error(errors, "org", T_error_org ) );
        add_key_pair(
                true, false, "welcome_message", T_welcome_message, welcome_message, identity,
                get_error(errors, "welcome_message", T_error_org ) );

        functionalitiesManager.closeSession();
	}

	private void add_groups_info( Division edit, EPerson edited_eperson ) throws WingException, SQLException
	{
        if (isAdmin())
        {
	        List member = edit.addList("eperson-member-of", List.TYPE_SIMPLE, "admin-field");
	...
	private void add_form_buttons(List identity, EPerson edited_eperson) throws WingException, SQLException
	{
	...
	private void add_constraints(List identity,  EPerson edited_eperson, Button submit_delete)
			throws SQLException, WingException {
		java.util.List<String> deleteConstraints = edited_eperson.getDeleteConstraints();
        ...
        if (deleteConstraints != null && deleteConstraints.size() > 0)
        {
        	...
        	Highlight hi = identity.addItem("eperson-delete-constraint",
        			"eperson-delete-constraint alert alert-error").addHighlight("error");
	...
	private Message get_error( java.util.List<String> errors, String key, Message err_msg) {
    ...
    public void addBody(Body body) throws WingException, SQLException, AuthorizeException
	{
	    ...
	    Division signed = edit.addDivision("signed-licenses", "well well-light");
	...
	private void add_signed_licenses(Division profile, EPerson edited_eperson) throws WingException {
		...
		try {
			...
			java.util.List<LicenseResourceUserAllowance> licenses = functionalityManager.getSignedLicensesByUser(edited_eperson.getID());
