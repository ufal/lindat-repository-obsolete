...
public class EditItemMetadataForm extends AbstractDSpaceTransformer {
    	...
    	private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(EditItemMetadataForm.class);
        ...
        // DIVISION: main
        Division main = body.addInteractiveDivision("edit-item-status", contextPath+"/edit/item", Division.METHOD_POST,"primary administrative item");
        ...
        if (!editingTemplateItem)
        {
          ...
          org.dspace.app.xmlui.wing.element.Item optItem = options.addItem(); 
          optItem.addHighlight("bold").addXref(tabLink, ViewItem.T_option_metadata);
        }
        ...
        addForm.setHead(T_head1);
                ...
                main.addHidden("eperson-continue").setValue(knot.getId());
                main.addHidden("itemID").setValue(itemID);
        }
	...
	public static boolean allowedField(String name) {
		try {
			...
			DCInputSet dcInputs = inputsReader.getInputs("");
			DCInputSet dcInputsExtra = inputsReader.getInputsExtra("");
			for (DCInputSet inputs : new DCInputSet[] { dcInputs, dcInputsExtra }) {
				for (int i = 0; i < inputs.getNumberPages(); i++) {
					DCInput[] pageInputs = inputs.getPageRows(i, false, false);
					for (int row = 0; row < pageInputs.length; row++) {
						String fullName = pageInputs[row].getSchema()
								+ "."
								+ pageInputs[row].getElement()
								+ (pageInputs[row].getQualifier() != null ? "."
